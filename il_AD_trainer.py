"""The trainer class for the adversarial domain adaptation,
a pretrained model will be loaded together with its data_loader.
RGB images will be sampled from the target domain without any labels available"""
...
from models.visionSafeAC import VisionNaiveMultihead, VisionNaiveRandomization, VisionCompleteMultiHead, VisionAdversarialAdaptActor
...
class IL_Trainer_CARLA_VisionAdversarialAdaptationAC(IL_Trainer_CARLA_VisionSafeAC):
    ...
    def __init__(self, carla_params,
        ...
        @param agent_params:
        """
        self.pretrain_encoder_path = PRETRAIN_LIGHTS
    ...
    def initialize_agent(self, comment, **kwargs):
        ...
        pretrain_agent = pretrain_class(**pretrain_agent_params)
        ...
        self.agent = VisionAdversarialAdaptActor(pretrained_agent = pretrain_agent, **kwargs)
    ...
    def train_module(self, module, global_step):
        ...
        self.agent.switch_mode(new_mode = "discriminator")
        d_info = module.fit(train_dataset=self.replay_buffer if self.agent.mode == 'discriminator' else self.replay_buffer.source_buffer,
                          n_epochs= (self.n_training_per_epoch if global_step > 0 else self.n_initial_training_epochs) * 10,
                          global_step=global_step)
        
        for mode, values in d_info.items():
            self.writer.do_logging(values, global_step=global_step, mode=mode)
        
        self.agent.switch_mode(new_mode = "actor")
        a_info = module.fit(train_dataset=self.replay_buffer if self.agent.mode == 'discriminator' else self.replay_buffer.source_buffer,
                          n_epochs= self.n_training_per_epoch if global_step > 0 else self.n_initial_training_epochs,
                          global_step=global_step)
        
        for mode, values in a_info.items():
            self.writer.do_logging(values, global_step=global_step, mode=mode)
    ...
    def training_loop(self, n_epochs: int):
        ...
        def make_stop_flag(consecutive_steps = 1, success_laps = 4):
            ...
            def f_stop_flag(evaluate_res):
                ...
                completed_laps = min([evaluate_res[cur_map['name']]["completed_laps"] for cur_map in self.domain_list + self.target_domains])
        ...
        # self.evaluate_agent(eval_domains = self.domain_list + self.target_domains)
        ...
        try:
            for global_step in range(self.starting_step, n_epochs):
                ...
                if global_step % self.eval_freq == 0 and self.agent.mode == "actor":
                    evaluate_res = self.evaluate_agent(eval_domains = self.domain_list + self.target_domains, global_step = global_step)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--target_domain_len", type = int, default = 5000) # the length of total trajectory sampled from the target domain
    ...
    config_path = "./config/VisionAdversarialAdapt.yaml"
