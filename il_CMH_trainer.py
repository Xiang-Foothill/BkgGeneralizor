"""The trainer for the Complete Multihead Model where the softgating mechanism is applied and 
the classifier is now used without the assumption that the agent has perfect knowledge about the environment"""
...
from models.visionSafeAC import VisionNaiveMultihead, VisionNaiveRandomization, VisionCompleteMultiHead
...
from il_NR_trainer import DOMAIN1, DOMAIN2, DOMAIN4, DOMAIN5, DOMAIN6, DOMAIN7, DOMAIN8, FULL_EVALUATION_LIST

class IL_Trainer_CARLA_VisionCompleteMultiHeadAC(IL_Trainer_CARLA_VisionSafeAC):
    ...
    def __init__(self, carla_params,
        ...
        @param agent_params:
        """
        self.pretrain_encoder_path = PRETRAIN_ENCODER9
        ...
        self.initialize_agent(comment=comment, heads_num = len(self.domain_list),  **agent_params)

        if to_reload:
            self.agent.load(path=Path(__file__).resolve().parent / 'model_data',
                           name=comment)
            self.agent.to_decision_training() # if in the model is reloaded from a pretrained one, it is by default the classifier has been tuned well, and only the classifier layer needs to be tuned
        
        logger.info(f"The current mode of the model is {self.agent.mode}")
        ...
        # an extra parameter
        self.randomnizor = linProgRandomnizer(final_percent=0.0, debug = False, mode = "constant") # set debug to false to speed up rendering
        transform = {"camera": self.randomnizor.randomnize}
    ...
    def initialize_agent(self, comment, heads_num, **kwargs):
        logger.debug(f"{kwargs}")
        if self.pretrain_agent_model == "VisionNaiveRandomization":
            pretrain_class = VisionNaiveRandomization
        else:
            pretrain_class = VisionNaiveRandomization

        pretrain_agent = pretrain_class(**kwargs)
        ...
        self.agent = VisionCompleteMultiHead(pretrained_agent = pretrain_agent, heads_num = heads_num, **kwargs)
    ...
    def label_domain_v(self, ob, domain):
        ...
        domain_v = np.zeros(shape = (len(self.domain_list), ), dtype = np.float32)
        if domain in self.domain_list: # if the domain is not in the domain list, return a null probability vector
            domain_index = self.domain_list.index(domain)
            domain_v[domain_index] = 1.
        ob["domain_v"] = domain_v
        return ob
    ...
    def sample_trajectory(self, domain, beta: float, pbar: Optional['tqdm'] = None,
        ...
        while traj_len < max_traj_len:
            ...
            self.label_domain_v(ob, domain) # label the carla observation the domain probability vector
    ...
    def training_loop(self, n_epochs: int):
        ...
        if not self.to_reload: # if the classifier is not part of the loaded model, train it from scratch
            ...
            try:
                for global_step in range(0, self.classifier_epochs):
                    logger.info(f"Epochs {global_step} / {self.classifier_epochs} for the classifier")

                    cur_beta = 0.95 # when training the classifier, treat the module decision as pure noise
                ...
                self.agent.to_decision_training() # switch the agent's mode to decision training
            finally:
                logger.info("------- The training for the classifier ends ---------")
            
        """start the agent training"""
        logger.info("--------------Start training the the decision layers-------------")
        try:
            for global_step in range(self.starting_step, n_epochs):
                ...
                cur_beta = self.init_beta * self.beta ** np.ceil(global_step - self.starting_step / self.beta_decay_freq)
                logger.info(f"the compensated beta value for the spawning this time is {cur_beta}")
                self.sample_trajectories(beta = cur_beta,
                                         total_length=self.initial_traj_len if global_step - self.starting_step <= 2 else self.eps_len,
                                         global_step=global_step)
                ...
                if global_step % self.eval_freq == 0:
                    evaluate_res = self.evaluate_agent(eval_domains = self.domain_list + self.eval_domain_list, global_step = global_step)
    ...
    def sample_trajectories(self, beta: float, total_length=None, global_step=None):
        ...
        with tqdm(total=total_length, desc='Sampling', unit='steps') as pbar:
            while batch_traj_len < total_length:
                ...
                domain = self.sample_domain(global_step)
                traj_len = self.sample_trajectory(domain = domain, beta = beta, pbar=pbar, max_traj_len=max_traj_len)
    ...
    def sample_domain(self, global_step):
        ...
        if (not self.evaluation_list[self.domain_list[0]['name']]) or (global_step - self.starting_step) == 0:
    ...
    def evaluate_agent(self, eval_domains, global_step, max_laps = 7):
        ...
        def one_iteration_test(eval_domain):
            ...
            while not truncated and completed_laps <= max_laps:
                ...
                self.label_domain_v(ob, eval_domain)
            ...
            # ---- Plot domain probabilities ----
            domain_probs_array = np.stack(domain_probs_list, axis=0)  # shape: [time, heads_num]
            time_steps = np.arange(domain_probs_array.shape[0])

            fig, ax = plt.subplots(figsize=(10, 4))
            heads_num = domain_probs_array.shape[1]

            for k in range(heads_num):
                domain_name = self.domain_list[k]['name']
                ax.plot(time_steps, domain_probs_array[:, k], label=f'{domain_name}', linewidth=2)

            ax.set_title(f"Domain probabilities over time for the evaluation domain {eval_domain['name']}")
            ax.set_xlabel("Time Step")
            ax.set_ylabel("Probability")
            ax.set_ylim(0, 1)
            ax.legend(loc='upper right')
            ax.grid(True)
            self.writer.add_figure(tag=f'val/domain_probs/{eval_domain["name"]}', figure=fig, global_step=global_step)
...
if __name__ == '__main__':
    ...
    trainer_cls = IL_Trainer_CARLA_VisionCompleteMultiHeadAC
