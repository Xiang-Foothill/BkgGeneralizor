...
DOMAIN5 = {"name": "forests", "map_name": L_TRACK_BARC5, "weatherID" : 4}
DOMAIN6 = {"name": "bus stops", "map_name": L_TRACK_BARC6, "weatherID" : 4}
...
class IL_Trainer_CARLA(ABC):
    ...
    def gnz_evaluation(self, global_iterations = 1, TOWN_LISTS = [L_TRACK_BARC, L_TRACK_BARC1, L_TRACK_BARC2], WEATHER_IDs = [0, 1, 2], max_laps = 5):
...
class IL_Trainer_CARLA_VisionSafeAC(IL_Trainer_CARLA_SafeAC):
    ...
    def evaluate_randomBkg(self, global_step):
        logger.info("Evaluating the agent with background randomization")
        self.agent.reset()
        self.agent.eval()
        # ob, info = self.env.reset(options={'render': True})
        ob, info = self.env.reset(options={'controller': self.expert, 'spawning': 'fixed'})
        ...
        if not hasattr(self, "randomnizor"):
            logger.info("No randomnizer is detected, initiate a new randomnizer")
            self.randomnizor = BkgRandomnizer(transfer_percentage=0.9)
        ...
        while not truncated:
            ...
            self.randomnizor.change_obs(ob, ifTest = True)
        ...
        self.writer.do_logging({
            ...
            'std_lap_time': np.std(lap_times[1:]) if len(lap_times) > 1 else np.nan,
            'completed_laps': completed_laps,
        }, global_step=global_step, mode='randomBkg_val')
        ...
        self.writer.add_figure(tag='randomBkg_val', figure=fig, global_step=global_step)

        return completed_laps
    
    def training_loop(self, n_epochs: int):

        def make_stop_flag(consecutive_steps = 12, success_laps = 10):
            """early stop mechnism, if we have consecutive evaluations with 
            completed laps larger than success_laps, the experiment is called to early stop,
            i.e. the system is recognized to converge to stable behavior"""
            history_eval = []
...
class IL_Trainer_CARLA_VisionSafeAC_randomFetch(IL_Trainer_CARLA_VisionSafeAC):
    def __init__(self, carla_params,
        ...
        # an extra parameter
        self.randomnizor = linProgRandomnizer(final_percent= augment_percent, debug = False, mode = "linear") # set debug to false to speed up rendering
...
class IL_Trainer_CARLA_VisionSafeAC_Contrast(IL_Trainer_CARLA_VisionSafeAC):
    def __init__(self, carla_params,
                       expert_cls,
                       initial_traj_len=1024,
                       eps_len=1024,
        ...
        # an extra parameter
        self.randomnizor = ContrastRandomnizer() # set debug to false to speed up rendering
        transform = {"positive": self.randomnizor.randomnize,
                     "negatives": self.randomnizor.negatives_randomnize}

        self.replay_buffer: 'data_util.EfficientReplayBuffer' = None
        self.replay_buffer = data_util.EfficientContrastReplayBufferPN(maxsize=replay_buffer_maxsize,
                                                               lazy_init=True,
                                                               transform = transform
                                                               )
    ...
    def initialize_agent(self, comment, **kwargs):
        logger.debug(f"{kwargs}")
        self.agent = visionSafeAC.VisionContrastiveAC(**kwargs)
    
class IL_Trainer_CARLA_VisionNaiveRandomizationAC(IL_Trainer_CARLA_VisionSafeAC):
    ...
    def __init__(self, carla_params,
        ...
        self.domain_list = [DOMAIN1, DOMAIN4]
        ...
        if to_reload:
            ...
            profile_path = Path(__file__).parent.parent / 'training_profiles' / f"{self.agent.model_name}_{self.comment}_training_profile.pkl"
            if profile_path.exists():
                with open(profile_path, 'rb') as f:
                    profile_data = pickle.load(f)
                self.beta = profile_data.get('beta', self.beta)
                self.starting_step = profile_data.get('cur_epoch', self.cur_epoch)
                self.evaluation_list = profile_data.get('evaluation_list', self.evaluation_list)
    ...
    def train_module(self, module, global_step):
        logger.info(f"Training {module.__class__.__name__}...")
        info = module.fit(train_dataset=self.replay_buffer.D_pos,
                          n_epochs=self.n_training_per_epoch if global_step > 0 else self.n_initial_training_epochs,
                          global_step=global_step)
        for mode, values in info.items():
            self.writer.do_logging(values, global_step=global_step, mode=mode)
    ...
    def sample_trajectory(self, beta: float, pbar: Optional['tqdm'] = None,
        ...
        @return:
        """
        domain = np.random.choice(self.domain_list)
    ...
    def training_loop(self, n_epochs: int):
        ...
        def make_stop_flag(consecutive_steps = 3, success_laps = 10):
            ...
            def f_stop_flag(evaluate_res):
                ...
                completed_laps = min([evaluate_res[cur_map]["completed_laps"] for cur_map in evaluate_res.keys()])
        ...
        profile_path = profile_dir / f"{self.agent.model_name}_{self.comment}_training_profile.pkl"
        ...
        try:
            for global_step in range(self.starting_step, n_epochs):
                ...
                # self.sample_trajectory(beta=self.beta ** np.ceil(global_step / self.beta_decay_freq),
                #                        max_traj_len=self.initial_traj_len if global_step == 0 else self.eps_len)
                cur_beta = self.beta ** np.ceil(global_step / self.beta_decay_freq)
                ...
                if self.no_saving:
                    continue
                if global_step % self.eval_freq == 0:
                    evaluate_res = self.evaluate_agent()
                    # self.evaluate_randomBkg(global_step=global_step)
    ...
    def sample_trajectories(self, beta: float, total_length=None, global_step=None):
        ...
        with tqdm(total=total_length, desc='Sampling', unit='steps') as pbar:
            while batch_traj_len < total_length:
                traj_len = self.sample_trajectory(beta, pbar=pbar, max_traj_len=min(1024, total_length - batch_traj_len))
                batch_traj_len += traj_len
                n_resets += 1
        self.writer.do_logging({f'failure_rate': (n_resets - 1) / total_length}, global_step=global_step, mode='train')
        return batch_traj_len
    
    def evaluate_agent(self, max_laps = 15):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--freeze_test")
    ...
    if params['observe'] == 'camera':
        if params["latent"]:
            ...
            config_path = "./config/visionSafeAC.yaml"
    elif params["observe"] == 'state':
        config_path = "./config/SafeAC.yaml"
    ...
    if params['observe'] == 'state':
        ...
        elif params["randomnize"] == "contrast":
            trainer_cls = IL_Trainer_CARLA_VisionSafeAC_Contrast
